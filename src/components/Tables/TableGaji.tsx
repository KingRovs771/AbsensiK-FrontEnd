...
const TableDepartements = () => {
  ...
  const handleDepartmentDelete = async (DepartmentsId: string) => {
    try {
      const DepartmentsResponse = await fetch(
        `http://localhost:8080/v1/departements/deleteDepartements/${DepartmentsId}`,
        {
          method: "DELETE",
          headers: {
      ...
      setDepartements(
        dataDepartements?.filter(
          (departements) => departements.departments_id !== DepartmentsId
        )
      );
    } catch (error) {
      setError(
        ...
        {dataDepartements && dataDepartements.length > 0 ? (
          dataDepartements.map((departements, index) => (
            ...
            >
              ...
              <div className="flex items-center gap-3 p-2.5 xl:p-5">
                <div className="flex-shrink-0 text-center ">
                  <p className="text-black dark:text-white">Mei</p>
                </div>
              </div>

              <div className="flex items-center justify-center p-2.5 xl:p-2">
                <p className="text-black dark:text-white text-center">Mei</p>
              </div>
              <div className="flex items-center justify-center p-2.5 xl:p-2">
                <p className="text-black dark:text-white text-center">2025</p>
              </div>
              <div className="flex items-center justify-center p-2.5 xl:p-2">
                <p className="text-black dark:text-white text-center">28</p>
              </div>
              <div className="flex items-center justify-center p-2.5 xl:p-2">
              ...
              <div className="hidden items-center justify-center p-2.5 sm:flex xl:p-5">
                <button
                  onClick={() =>
                    handleDepartmentDelete(departements.departments_id)
                  }
