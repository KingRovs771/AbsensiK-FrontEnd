...
const TableTipePotongan = () => {
  const [dataTipePotongan, setTipePotongan] = useState<Tipe_potongan[]>([]);
  ...
  useEffect(() => {
    const fetchTipePotongan = async () => {
      try {
        ...
        const apiResponse: ApiResponse<Tipe_potongan[]> =
          await tipePotonganResponse.json();
        if (apiResponse.Status === "Success") {
          setTipePotongan(apiResponse.Data);
        } else {
  ...
  const handleTipePotonganDelete = async (TipePotonganId: number) => {
    try {
      const TipePotonganResponse = await fetch(
        `http://localhost:8080/v1/tipePotongan/deleteTipePotongan/${TipePotonganId}`,
        {
          method: "DELETE",
          headers: {
      ...
      toast.success("Berhasil Menghapus Data Tipe Potongan");
      setTipePotongan(
        dataTipePotongan?.filter(
          (Tipe_potongan) => Tipe_potongan.tipe_potongan_id !== TipePotonganId
        )
      );
    } catch (error) {
      setError(
    ...
    <>
      ...
      <div className="rounded-sm border border-stroke bg-white px-12 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        ...
        <a
          ...
          href="/masterdata/tipePotongan/formInput"
        >
          <svg
          ...
          <span>Insert Tipe Potongan</span>
        </a>
        ...
        <div className="flex flex-col">
          ...
          {dataTipePotongan && dataTipePotongan.length > 0 ? (
            dataTipePotongan.map((tipePotongan, index) => (
              <div
                className={`grid grid-cols-3 sm:grid-cols-5 ${
                  index === dataTipePotongan.length - 1
                    ? ""
                    : "border-b border-stroke dark:border-strokedark"
                }`}
                key={tipePotongan.tipe_potongan_id}
              >
                ...
                <div className="hidden items-center justify-center p-2.5 sm:flex xl:p-5">
                  <a
                    ...
                    href={`/masterdata/tipePotongan/formUpdate/${tipePotongan.tipe_potongan_id}`}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="18"
                ...
                <div className="hidden items-center justify-center p-2.5 sm:flex xl:p-5">
                  <button
                    ...
                    onClick={() =>
                      handleTipePotonganDelete(tipePotongan.tipe_potongan_id)
                    }
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
...
export default TableTipePotongan;
