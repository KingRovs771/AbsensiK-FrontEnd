    ...
    const fetchRadius = async () => {
      try {
        const radiusResponse = await fetch(
          "http://localhost:8080/v1/radius/AllRadius"
        );
  ...
  const handleRadiusDelete = async (RadiusId: number) => {
    try {
      const DepartmentsResponse = await fetch(
        `http://localhost:8080/v1/radius/deleteRadius/${RadiusId}`,
        {
          method: "DELETE",
          headers: {
    ...
    <>
      ...
      <div className="rounded-sm border border-stroke bg-white px-12 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        ...
        <a
          ...
          href="/masterdata/radius/formInput"
        >
          <svg
        ...
        <div className="flex flex-col">
          ...
          {dataRadius && dataRadius.length > 0 ? (
            dataRadius.map((radius, index) => (
              <div
                className={`grid grid-cols-3 sm:grid-cols-7 ${index === dataRadius.length - 1 ? "" : "border-b border-stroke dark:border-strokedark"}`}
                key={radius.radius_id}
              >
                ...
                <div className="flex items-center gap-3 p-2.5 xl:p-5">
                  <div className="flex-shrink-0 text-center ">
                    <p className="text-black dark:text-white">
                      {String(radius.name_location).substring(0, 10)}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-center p-2.5 xl:p-2">
                  <p className="text-black dark:text-white">
                    {String(radius.latitude).substring(0, 10)}
                  </p>
                </div>

                <div className="flex items-center justify-center p-2.5 xl:p-2">
                  <p className="text-black dark:text-white">
                    {String(radius.longitude).substring(0, 10)}
                  </p>
                </div>
                ...
                <div className="hidden items-center justify-center p-2.5 sm:flex xl:p-5">
                  <a
                    ...
                    href={`/masterdata/radius/formUpdate/${radius.radius_id}`}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="18"
                ...
                <div className="hidden items-center justify-center p-2.5 sm:flex xl:p-5">
                  <button
                    onClick={() => handleRadiusDelete(radius.radius_id)}
