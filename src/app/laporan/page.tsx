...
// ===================================================================
// 2. Komponen Modal Pemilih Periode (DIPERBARUI)
// ===================================================================
const ReportPeriodModal = ({
  isOpen,
  onClose,
  onPrint, // <-- PERBAIKAN 1: Mengubah nama prop dari onProcess menjadi onPrint
  reportTitle,
}: {
  isOpen: boolean;
  onClose: () => void;
  onPrint: (month: string, year: string, monthName: string) => void; // <-- Mengubah nama tipe prop
  reportTitle: string;
}) => {
  ...
  const handleProcessClick = () => {
    ...
    onPrint(month, year, monthName); // <-- PERBAIKAN 2: Memanggil fungsi onPrint yang benar
    onClose();
  };
...
const LaporanPage: React.FC = () => {
  ...
  const handlePrepareReport = async (
    ...
    setIsLoading(true);
    const toastId = toast.loading("Mengambil data laporan, mohon tunggu...");

    try {
      const params = new URLSearchParams();
      params.append("type", currentReport.type);
      if (currentReport.type !== "users") {
        params.append("month", month);
        params.append("year", year);
      }

      const apiUrl = `http://localhost:8080/v1/report/generateReport?${params.toString()}`;
      ...
      if (!response.ok || apiResponse.Status !== "Success") {
        throw new Error(apiResponse.Message || `Gagal mengambil data`);
      }

      if (apiResponse.Data && apiResponse.Data.length > 0) {
        toast.update(toastId, {
          render: "Data ditemukan! Menampilkan pratinjau...",
          type: "success",
          isLoading: false,
          autoClose: 2000,
        });
        ...
        setReportPeriod(
          currentReport.type === "users" ? "Semua Data" : `${monthName} ${year}`
        );
        setView("print");
      } else {
